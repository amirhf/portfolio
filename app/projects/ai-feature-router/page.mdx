...
<ProjectPage>
  ...
  >
    ...
    <ProjectServiceTags tags={['AI Product Integration', 'Distributed Systems & Microservices']} />
  ...
  <ProjectSection title="Business outcomes" eyebrow="Impact">
    <ProjectOutcomes>
      <ProjectOutcome label="Latency target" value="Sub-100ms p95">
        Go orchestration, cache-first routing, and decoupled inference keep search interactions responsive.
      </ProjectOutcome>
      <ProjectOutcome label="Cost control" value="Tiered AI routing">
        Requests flow through cache, local models, and cloud fallback based on confidence and complexity.
      </ProjectOutcome>
      <ProjectOutcome label="Product fit" value="SaaS-ready">
        Tenant-aware search and feature routing patterns map cleanly to marketplace and retail teams.
      </ProjectOutcome>
    </ProjectOutcomes>
  </ProjectSection>
  ...
  <ProjectSection title="The AI Feature Router (Cost & Latency Control)">
    ...
    />
    <p>
      Directly connecting users to LLM APIs is a recipe for unmanageable costs. This system implements a <strong>Multi-Tiered Routing Strategy</strong> to optimize for economic viability.
    </p>
    <ul>
      <li>
        <strong>Tier 1 (Edge/Cache):</strong> Semantic caching checks if a similar query (Cosine Similarity &gt; 0.95) was recently answered, returning results in &lt;10ms.
      </li>
      <li>
      ...
      <li>
        <strong>Tier 3 (Cloud Fallback):</strong> Only when the router detects high ambiguity or low confidence does it escalate the request to a premium model (e.g., GPT-4o), ensuring high quality without the high bill.
      </li>
    </ul>
  </ProjectSection>
  ...
  <ProjectSection title="Hybrid Search & Multi-Tenancy">
    <p>
      <strong>Hybrid Search Fusion:</strong> Retail requires precision. Pure vector search can fail on specific product codes, while keyword search misses semantic context. This system implements <strong>Reciprocal Rank Fusion (RRF)</strong>, mathematically merging Dense Vectors (OpenCLIP) and Sparse Vectors (BM25) to deliver results that are both conceptually relevant and technically accurate.
    </p>
    <p>
      <strong>SaaS-Ready Multi-Tenancy:</strong> The architecture supports strict data isolation. Go middleware injects tenant context into every request, and Qdrant enforces payload filtering at the engine level. This ensures that in a shared infrastructure, Tenant A never sees Tenant B's inventory.
    </p>
  </ProjectSection>
  ...
  <ProjectSection title="Engineering Decisions (ADR)">
    ...
    <p>
      ...
      Qdrant was selected for its performance in high-concurrency scenarios and its first-class support for <strong>quantization</strong>. This allows us to serve millions of vectors from RAM on commodity hardware, significantly reducing the infrastructure overhead compared to other vector stores.
    </p>
  </ProjectSection>
</ProjectPage>
